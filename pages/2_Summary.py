...
def render_main_content():
    st.title("DEMO-GPT: Email Summary")
    ...
    st.info("""
            This agent is an email assistant that helps with email-related tasks such as 
...
def handle_user_input(client, system_message):
    ...
    if st.button("Example: Coffee Machine Email Thread", type='primary'):
        prompt = examples['summary']['example_summary'] 
...
def reset_session_state_button():
    if st.button("Reset chat history", type='secondary', key="reset_chat_summary"):
        st.session_state["messages"] = [
            {"role": "assistant", "content": "Feel free to copy in content, type or use the example to start."}
        ]
        st.session_state["response"] = None
...
def main():
    ...
    system_message = {
        "role": "system",
        "content": examples['summary']['system_prompt_summary']
    }
